...
Is delay influenced by departure time? Let's look at `B6` which has a high median of `DEP_DELAY`

```{r}
planes %>% 
  filter(OP_CARRIER == "B6") %>% 
  ggplot(aes(DEP_TIME,DEP_DELAY,  color = OP_CARRIER)) + 
  geom_point()
```
