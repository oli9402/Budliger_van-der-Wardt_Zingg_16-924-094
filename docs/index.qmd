---
title: "Analysing Flights"
author: 
  - name: Lenny Budlinger
  - name: Thomas van der Wardt 
  - name: Oliver Zingg
format:
  html:
    self-contained: true
    theme: cosmo  # or "default", "flatly", "journal", etc.
    full-width: true
    mainfont: "sans-serif"
    monofont: "monospace"
    code-fold: true
...
# We can see that the second model with the `TAXI_OUT` variable significantly improves the model fit. Let's see if we can improve the model further by adding the airlines as predictor.
...
print(summary(lm.arrival.4)$coefficients %>% round(digits = 7))
...
planes %>%
  ...
  facet_wrap(~ OP_CARRIER_Tier) +
  labs(
    title = "After cubic root Transformation",
    x = "Value",
    y = "Density"
  ) +
...
#plot(lm.arrival.5)
print(summary(lm.arrival.5)$coefficients %>% round(digits = 7))
...
```{r}
#| output: FALSE
#| code-summary: "Show Code of Model"
#| 
...
After downsampling, the SVM model achieved an accuracy of 58.4%, below the baseline of 64.02%, but sensitivity improved to 64.03%, indicating better detection of delayed flights—the primary goal. Specificity dropped to 55.15%, reflecting more false positives. Despite lower accuracy, balanced accuracy rose to 59.59% and kappa improved to 0.18, showing a better balance between classes. This trade-off is acceptable given the focus on delays. The next step is to apply recursive feature elimination (RFE) and rerun the model.
...
```
After feature selection, sensitivity is 84.8%, slightly improving the detection of delayed flights, while overall accuracy dropped slightly to 47.3%.
...
# Conclusion 
The dataset used in this task demonstrates limited predictive capability for determining whether a flight will experience a delayed departure or arrival. Two main strategies were explored for feature selection. One included all available features, while the other, more aligned with practical business considerations, limited the selection to features known before the aircraft begins to move. The latter approach was evaluated using a Support Vector Machine (SVM) model. However, even after applying feature engineering, the predictions produced by the SVM were not sufficiently accurate. Therefore, if an airport intends to provide travelers with advance notice of likely delays at their destination, incorporating additional features or experimenting with alternative models is necessary.
For more reliable prediction we conclude that it would be vastly beneficial to collect additional features like weather or general airport traffic. 

## Usaged of AI
We used ChatGPT to assist with debugging code (such as data wrangling, plotting) and questions related to quarto, markdown. In one case we used ChatGPT to group the airlines into Tier level as outlined in Chapter 1.  
