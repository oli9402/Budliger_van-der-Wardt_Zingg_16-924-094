...
```{r}
planes_intial_svm <- planes

hist(planes_intial_svm$ARR_DELAY,
...
plot(density(plane_inital_svms$ARR_DELAY, na.rm = TRUE),
...
sum(is.na(planes_intial_svm$DELAYED))
...
At this stage, the final set of features is defined.  The remaining non-categorical features are then normalized to ensure they are on a consistent scale.
...
We will choose  z-score scaling, see https://forecastegy.com/posts/does-svm-need-feature-scaling-or-normalization/

```{r}
cols_to_normalize <- c("CRS_ELAPSED_TIME", "DISTANCE", 
                       "DEP_SIN", "DEP_COS", 
                       "ARR_SIN", "ARR_COS")

planes_svm[cols_to_normalize] <- scale(planes_svm[cols_to_normalize])

scaling_params <- attributes(scale(planes_svm[cols_to_normalize]))
```

Now the data is ready train and test a svm to predict DELAYED.

## SVM Modeling

To conduct an initial performance evaluation, the SVM model is trained using the caret package with the e1071 implementation. Given the size of the dataset, a 30% subset is used to reduce computational time during training. The tuneLength parameter is set to 3, combined with 5-fold cross-validation, to enable a quick, preliminary analysis of model performance while minimizing training overhead.
...
The Support Vector Machine (SVM) model achieved an accuracy of 65.12%, which is only marginally higher than the no-information rate of 64.02%. While the model performs well in identifying non-delayed flights, as indicated by a high specificity of 91.1%, its sensitivity is considerably low at 18.8%, reflecting a significant challenge in detecting delayed flights. The balanced accuracy of 55.0% and a modest Cohen’s kappa of 0.116 further underscore the model’s limited effectiveness in handling class imbalance. To improve performance, particularly in predicting delays, the next step involves applying downsampling using the parameter sampling = "down". This aims to balance the class distribution and enhance the model’s ability to detect delayed flights.
...
subset_index <- createDataPartition(svm_train_data$DELAYED, p = 0.5, list = FALSE)
svm_train_data <- svm_train_data[subset_index, ]
...
After down-sampling, the SVM model achieved an accuracy of 59.6%, below the baseline (63.0%). Sensitivity improved significantly (63.5%) compared to the previous model, indicating better identification of delayed flights, though specificity decreased (57.3%). Despite higher balanced accuracy (60.4%) and improved kappa (0.194), the overall predictive performance remains modest. The trade-off of increased sensitivity with lower accuracy and specificity should be carefully considered depending on business priorities. The next step is to test recursive feature selection to remove unhelpful features, and to run the test without downsampling.


```{r}
set.seed(123)
svm_train_index <- createDataPartition(planes_svm$DELAYED, p = 0.7, list = FALSE)
train_full      <- planes_svm[svm_train_index, ]
test_data       <- planes_svm[-svm_train_index, ]

slice30_idx <- createDataPartition(train_full$DELAYED, p = 0.3, list = FALSE)
slice30     <- train_full[slice30_idx, ]

slice90_idx <- createDataPartition(slice30$DELAYED, p = 0.9, list = FALSE)
slice90     <- slice30[slice20_idx, ]

slice20$DELAYED <- factor(slice20$DELAYED, levels = c("0","1"))
...
test_data$DELAYED <- factor(test_data$DELAYED, levels = c("0","1"))

x_train30 <- model.matrix(DELAYED ~ . - 1, data = slice30)
x_test   <- model.matrix(DELAYED ~ . - 1, data = test_data)

train_sel <- data.frame(x_train30[, selected_features, drop = FALSE],
                        DELAYED = slice30$DELAYED)
test_sel  <- data.frame(x_test[,   selected_features, drop = FALSE],
                        DELAYED = test_data$DELAYED)
...
preds    <- predict(svm_model, newdata = test_sel)
conf_mat <- confusionMatrix(preds, test_sel$DELAYED, positive = "1")
print(conf_mat)

```
Following feature selection using a Random Forest approach, the SVM model yielded an accuracy of 63.8%, only slightly above the baseline rate of 63.2%. However, sensitivity dropped significantly to 5.2%, indicating a substantial inability to identify delayed flights. In contrast, specificity remained high at 98.0%, showing that non-delayed flights were consistently classified correctly. The resulting balanced accuracy of 51.6% and a low Cohen’s kappa of 0.039 highlight the model’s limited discriminative power. These outcomes suggest that, despite the application of advanced feature selection, the model remains ineffective in reliably detecting flight delays.

## Interpretation
The SVM model implemented using the caret framework consistently underperforms in detecting delayed flights. While it maintains high specificity across all configurations, sensitivity remains low, limiting its utility for identifying delays. Neither downsampling nor feature selection yielded substantial improvements in balanced accuracy or discriminative power. Among the configurations tested, the initial model exhibited the best overall performance in terms of accuracy and specificity, despite its poor sensitivity. To improve results, future work should focus on addressing class imbalance through more advanced resampling strategies, refining the SVM kernel and hyperparameters, or exploring alternative classification approaches such as gradient boosting or ensemble methods that are better suited for imbalanced datasets.
