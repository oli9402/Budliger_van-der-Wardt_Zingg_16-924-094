...
plot(density(planes$ARR_DELAY, na.rm = TRUE),
...
sum(is.na(planes$DELAYED))
length(planes$DELAYED)


planes$DELAYED <- ifelse(planes$ARR_DELAY > 0, 1, 0)

barplot(table(planes$DELAYED),
        names.arg = c("On Time (0)", "Delayed (1)"),
        col = c("lightgreen", "salmon"),
        main = "Distribution of Flight Delays",
        ylab = "Number of Flights")
     
pie(table(planes$DELAYED),
    labels = c("On Time", "Delayed"),
    col = c("lightblue", "orange"),
    main = "Proportion of Delayed Flights")
    
mean(planes$DELAYED, na.rm = TRUE)
...
First, we use e1071 on the whole dataset. Because the dataset is quite large we initially want to make a quick performance analysis and therefore use tuneLenght = 1 with 5-fold cross validation. Also we first use a subset of the dataset with 50% to speed up computation for training.
...
class_weights <- ifelse(svm_train_data$DELAYED == "1", 2, 1)
...
Next step is to sampling = smote to make our train data more balanced.
And then make our own dataframe right at the beginning of the code and take the airline classifier into our code on our on dataframe.
