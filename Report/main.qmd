...
To incorporate the features OP_CARRIER_Tier and OP_CARRIER into the model, one-hot encoding is applied. This transformation converts categorical variables into a numerical format suitable for input into the SVM classifier.

```{r}
planes_svm <- cbind(planes_svm, model.matrix(~ OP_CARRIER_Tier - 1, data = planes_svm))
planes_svm <- cbind(planes_svm, model.matrix(~ OP_CARRIER - 1, data = planes_svm))
```
